...
class RecipeView extends View {
  ...
  _errorMessage = `We couldn't find any recipe please try again with another ID 🙁`;
  ...
  addHandlerServings(handler) {
    this._parent.addEventListener('click', e => {
      ...
      const servingNumber = +btn.dataset.updateTo;
      if (servingNumber < 1) return;
      handler(servingNumber);
    });
  }
  ...
  _generateMarkup() {
    ...
    <figure class="recipe__fig">
          <img src="${this._recipe.imgUrl}" alt="${
      this._recipe.title
    }" class="recipe__img" />
        ...
        <div class="recipe__ingredients">
          ...
          <ul class="recipe__ingredient-list">
            ${this._recipe.ingredients
              .map(this._generateMarkupIngridents)
              .join('')}
          </ul>
        </div>
